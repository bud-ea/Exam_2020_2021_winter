...
string func_sdnf(std::vector<bool> f) {
    ...
    for (int i = 0; i < f.size(); i++) {// перебираем все значения функции
        // Пострить СДНФ можно только для исттиных знаений функции
        if (f.at(i)) { // сообщаем, что функция действительно имеет СДНФ
            has_s_func = true;
            func += "(";
            for (int j = 0; j < num; j++) { // проходим по таблице истинности
